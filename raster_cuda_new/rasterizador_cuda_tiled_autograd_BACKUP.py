...
class RasterizarTiledCUDA(torch.autograd.Function):
    @staticmethod
    def forward(ctx, mu, scale, theta, opacity, color, depth, H, W, tile_size, k_sigma, usar_preprocess_cuda):
        mu_c = mu.contiguous()
        ...
        # Preprocess CUDA: duplicateWithKeys + CUB sort + ranges.
        # Si no existe la funcion nueva, usa el fallback PyTorch.
...
def rasterizar_un_frame_cuda_tiled(params_frame, H, W, tile_size=16, k_sigma=3.5, usar_preprocess_cuda=True):
