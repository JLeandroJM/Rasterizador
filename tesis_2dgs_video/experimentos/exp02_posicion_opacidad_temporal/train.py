"""
exp02 — entrena modelo temporal con centro y opacidad polinomicos.
...
from modelo import Modelo2DGSTemporalPosOp
...
def crear_optimizador(modelo, lrs):
    return torch.optim.Adam([
        ...
        {'params': [modelo.opacidad_coefs], 'lr': lrs['opacidad']},
...
def guardar_heatmap_vida(vivos, ruta, titulo):
    """vivos: array (N, T) binario."""
    fig, ax = plt.subplots(figsize=(10, 6), dpi=100)
    ax.imshow(vivos, aspect='auto', cmap='inferno', interpolation='nearest')
    ax.set_xlabel("frame_idx")
    ax.set_ylabel("gaussiana #")
    ax.set_title(titulo)
    fig.tight_layout()
    fig.savefig(ruta)
    plt.close(fig)
...
def procesar_clip(nombre_clip, carpeta_clip, carpeta_salida, config, device):
    ...
    # heatmap de vida (lo distintivo de exp02)
    vivos, opacidades_completas = modelo.heatmap_vida(T, umbral=umbral_vida)
    guardar_heatmap_vida(vivos, os.path.join(carpeta_salida, "heatmap_vida.png"),
                          f"gaussianas vivas (opacidad > {umbral_vida}) — {nombre_clip}")
    ...
    escribir_metricas_json(
        ...
        exp="exp02_posicion_opacidad_temporal",
        clip=nombre_clip,
        num_frames=T,
        resolucion=[H, W],
        ...
        gaussianas_promedio_vivas=float(vivos.mean() * N),
    )
...
def main():
    ...
    resultados_dir = os.path.join(raiz, "resultados", "exp02_posicion_opacidad_temporal")
