"""
exp03 — entrena modelo full temporal (todos los parametros polinomicos).
...
from modelo import Modelo2DGSTemporalFull
...
def crear_optimizador(modelo, lrs):
    return torch.optim.Adam([
        ...
        {'params': [modelo.escala_coefs],   'lr': lrs['escala']},
        {'params': [modelo.theta_coefs],    'lr': lrs['theta']},
        {'params': [modelo.opacidad_coefs], 'lr': lrs['opacidad']},
        {'params': [modelo.color_coefs],    'lr': lrs['color']},
    ])
...
def procesar_clip(nombre_clip, carpeta_clip, carpeta_salida, config, device):
    ...
    modelo = Modelo2DGSTemporalFull(N, q, H, W, device,
    ...
    for it in range(iteraciones):
        ...
        opt.zero_grad()
        c, e, th, o_raw, k_raw, p = modelo.evaluar_en_t(t_norm)
        render = rasterizar_diferenciable(c, e, th, o_raw, k_raw, p, H, W)
        loss = calcular_loss(render, target, lambda_dssim=lambda_dssim)
        loss.backward()
        opt.step()
        # NOTA: no llamamos clampear_escala porque escala es polinomico aqui.
        # El clamp se hace dentro de evaluar_en_t() del modelo.
    ...
    with torch.no_grad():
        for fi in range(T):
            t_norm = fi / (T - 1) if T > 1 else 0.0
            c, e, th, o_raw, k_raw, p = modelo.evaluar_en_t(t_norm)
            render = rasterizar_diferenciable(c, e, th, o_raw, k_raw, p, H, W).clamp(0, 1)
...
def main():
    ...
    resultados_dir = os.path.join(raiz, "resultados", "exp03_full_temporal")
